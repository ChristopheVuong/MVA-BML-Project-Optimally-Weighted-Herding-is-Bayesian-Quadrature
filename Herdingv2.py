...
# def generate_herding_numba(kernel, gm, area, samples):
#     xa, xb, ya, yb = area
...
#     for xx in x:
#         for yy in y:
#             s = score_numba(xx, yy, kernel, gm, samples)
#             if(s > maxi):
#                 maxi = s
#                 x_max = xx
#                 y_max = yy
...
# @numba.jit(nopython=True, parallel=True)
# def herding_numba(num_samples, kernel, gm, area, samples=[]):
#     for k in range(num_samples):
#         print(k)
#         samples.append(generate_herding_numba(kernel, gm, area, samples))
#     return samples

# @numba.jit(nopython=True, parallel=True)
# def score_numba(xx,yy,kernel,gm, samples):
#     x = np.array([xx, yy])
#     s = 0
#     for k in range(len(gm.means)):
#         s += gm.weights[k]*E_Gaussian(x, kernel.covariance,
#                                       gm.means[k], gm.covariances[k])
