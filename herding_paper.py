...
def generate_samples(distrib, kernel, sequence, area, num_samples):
    ...
    expected_losses = herding_loss_next(distrib, kernel, sequence, samples)
    best_idx = np.argmin(expected_losses)
    return samples[best_idx,:], expected_losses[best_idx]

def herding_loss_next(distrib, kernel, sequence, samples):
    ...
    for i in range(n_samples):
        ...
        # print(samples[i,:])
        new_gram_col = kernel.evaluate(existing_samples)
