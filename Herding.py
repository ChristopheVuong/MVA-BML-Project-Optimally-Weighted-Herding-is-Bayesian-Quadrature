...
def E_Gaussian(x,sigma0,mu,sigma):
    ...
    return np.sqrt(np.linalg.det(new_sigma)/(np.linalg.det(sigma0)*np.linalg.det(sigma)))*exp/(2*np.pi)**(d/2)
...
def EE_Gaussian(kernel,gm):
    ...
    output = 0
    gamma = kernel.covariance
    inv_gamma = np.linalg.inv(gamma)
    det_gamma = nlin.det(gamma)
    for k in range(len(gm.means)):
        ...
        Q_k = np.linalg.inv(inv_gamma+inv_sigma_k)
        inv_S_k = inv_gamma + inv_sigma_k - inv_gamma@Q_k@inv_gamma
        S_k = np.linalg.inv(inv_S_k)
        
        det_Q_k = np.linalg.det(Q_k)
        det_S_k = np.linalg.det(S_k)
        
        a_k = S_k @ (inv_sigma_k - inv_gamma @ Q_k @ inv_sigma_k) @ mu_k
        # a_k = -(S_k@inv_sigma@Q_k@inv_sigma_k@mu_k)[None]
        print(a_k.shape)
        # exp = np.exp(-(mu_k.T@(inv_sigma_k-inv_sigma_k@Q_k@inv_sigma_k)@mu_k-a_k@inv_S_k@a_k.T)/2)
        exp = np.exp(-1/2 * (mu_k @ (2*inv_sigma_k - inv_sigma_k @ Q_k @ inv_sigma_k) @ mu_k.T - a_k @ inv_S_k @ a_k.T))
        output += gm.weights[k]*np.sqrt(det_Q_k*det_S_k/det_gamma) / det_sigma_k * exp
    return output / (2*np.pi)**(d / 2)
...
def MMD2(kernel,gm,samples):
    ...
    for sample in samples:
        for k in range(len(gm.means)):
            crossed_term += E_Gaussian(sample[None],kernel.covariance,gm.means[k],gm.covariances[k])
